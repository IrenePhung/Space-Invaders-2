...
import invaders.observer.GameClock;
...
public class GameWindow {
	...
	public GameWindow(GameEngine model){
        ...
        Label clockLbl = gameClock.getLabel();
        clockLbl.setLayoutX(10);
        clockLbl.setLayoutY(10);

        GameScore gameScore = new GameScore();
        Label scoreLbl = gameScore.getLabel();
        scoreLbl.setLayoutX(500);
        scoreLbl.setLayoutY(10);

        model.registerObserver(gameClock);
        model.registerObserver(gameScore);
        pane.getChildren().addAll(clockLbl, scoreLbl);
    }
    ...
    private void draw(){
        ...
        for (Renderable entity : model.getPendingToRemoveRenderable()){
            for (EntityView entityView : entityViews){
                if (entityView.matchesEntity(entity)){
                    // System.out.println(entity.getRenderableObjectName());
                    entityView.markForDelete();
                }
            }
        }
